...
def parse(html):
    ...
    if eod and len(eod["rows"]) > 1:
        r = _text_cells(eod["rows"][1])
        if len(r) >= 6:
            out["eod"] = {
                "date": r[0], "day": r[1], "over_short": _money(r[2]),
                "gross": _money(r[3]), "net": _money(r[4]), "paid_out": _money(r[5]),
            }

    # The Total Cash / Proof table has no caption; find it by its header.
    for t in tables:
        rows = t["rows"]
        if len(rows) > 1 and len(_text_cells(rows[0])) >= 4:
            hdr = [c.lower() for c in _text_cells(rows[0])]
            if "total cash" in hdr and "proof" in hdr:
                r = _text_cells(rows[1])
                out["cash"] = {"total_cash": _money(r[2]), "proof": _money(r[3])}
                break

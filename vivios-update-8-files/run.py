...
def main():
    ...
    for back in range(2, 8):
        ...
        if d0 in state["actuals_gross"] or d0 < "2026-08-19":   # nothing before launch
            continue
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        f.write(page)
    state["flag_count_history"] = [h for h in state["flag_count_history"]
                                   if h.get("log_date") != log_date]
    state["flag_count_history"].append(
        {"date": now.date().isoformat(), "log_date": log_date,
         "high": facts["flag_counts"]["high"], "medium": facts["flag_counts"]["medium"]})

...
def build(facts, flags, state, cleared=None, read_line=""):
    ...
    try:
        _d = dt.date.fromisoformat(log_date)
        nice_date = "%s %d, %d" % (_d.strftime("%B"), _d.day, _d.year)
    except ValueError:
        nice_date = log_date
    ...
    p.append('<header class="top"><div class="inner"><h1>Vivio\'s Warren, Owner Status</h1>'
             ...
             '</div></div></header><div class="wrap">' % (e(day_name), e(nice_date)))
    ...
    if s:
        big = m(s["gross"])
        beat = (s["t4_var_pct"] or 0) > 0
        note = ("%s the last four real %ss (%s) by %.1f%%. Forecast %s (%+.1f%%), last year %s (%+.1f%%)."
                % ("Beat" if beat else "Missed", day_name, m(s["trailing4"]), abs(s["t4_var_pct"] or 0),
                   m(s["forecast"]), s["fc_var_pct"] or 0, m(s["ly"]), s["ly_var_pct"] or 0))
        cls = "green" if beat else "amber"
    else:
    ...
    p.append('<div class="tile %s"><div class="lbl">%s %s sales</div><div class="big">%s</div>'
             '<div class="note">%s</div></div>' % (cls, e(day_name), e(log_date[5:]), big, note))
    ...
    p.append('<div class="card"><h2>Cash trail</h2><div class="body">')
    tenders = facts.get("tenders") or {}
    if facts.get("expected_deposit") is not None:
        dep = facts.get("deposit")
        proof = round(sum(tenders.values()), 2) if tenders else None
        total_cash = round(facts["expected_deposit"] + proof, 2) if proof is not None else None
        p.append('<div class="kv">')
        p.append('<div><span>Expected deposit %s</span><b>%s</b></div>'
                 % (e(log_date[5:]), m(facts["expected_deposit"])))
        p.append('<div><span>Counted deposit</span><b%s>%s</b></div>'
                 % (' style="color:#b3261e"' if dep is None else "",
                    m(dep) if dep is not None else "none entered"))
        p.append('<div><span>Counted by</span><b style="font-size:15px">%s</b></div>'
                 % e(facts.get("deposit_by") or "nobody"))
        p.append('<div><span>Total cash / proof</span><b style="font-size:15px">%s / %s</b></div>'
                 % (m(total_cash), m(proof)))
        p.append('</div>')
    if tenders:
        p.append('<p style="font-size:13px;color:#5d6b7a;margin:10px 0 0">Non cash tenders: '
                 + ", ".join("%s %s" % (e(k.replace("_", " ").title()), m(v)) for k, v in tenders.items())
                 + ". These sum to Proof, so the gap between Total Cash and Proof is the cash.</p>")
    if facts.get("mix"):
        p.append('<p style="font-size:13px;color:#5d6b7a;margin:6px 0 0">Sales mix: '
                 + ", ".join("%s %s" % (e(k.title()), m(v)) for k, v in facts["mix"].items()) + '.</p>')
    ly_ = facts.get("labor_yesterday")
    if ly_:
        hv = ly_["hours_vs_schedule"]
        p.append('<p style="font-size:13px;color:#5d6b7a;margin:6px 0 0">Labor %s: %.2f hours against '
                 '%.2f scheduled (%s%.2f), %s in pay, %.2f%% against a necessary %.2f%%.</p>'
                 % (e(log_date[5:]), ly_["hours"], ly_["sched_hours"], "+" if hv > 0 else "", hv,
                    m(ly_["pay"]), ly_["lc_pct"] or 0, ly_["necessary_lc_pct"] or 0))
    p.append('<div class="scroll" style="margin-top:12px">'

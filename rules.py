...
def evaluate(p, state, ly_sales, today=None):
    ...
    if dep_amt is None:
        ...
        state["cash"]["counted_deposits"].append(
            {"date": p["log_date"], "amount": dep_amt,
    ...
    ok_dates = {(day - dt.timedelta(days=k)).strftime("%m-%d-%Y") for k in (0, 1)}
    seen = {(f.get("date"), f.get("product")) for f in state["open_flags"]["price_increases"]}
    for row in p["price_history"]:
        if row["date"] not in ok_dates or row["pct"] is None:
            continue
        iso = dt.datetime.strptime(row["date"], "%m-%d-%Y").date().isoformat()
        ...
        if row["pct"] > cfg["price_increase_pct"]:
            ...
            flags.append(flag("medium", "C1d", "Steep price decrease, data check only.",
                              "%s %s, $%.2f to $%.2f, %.2f%%."
                              % (row["vendor"], row["product"], row["old"],
                                 row["new"], row["pct"])))
    ...
    # ---- new products ------------------------------------------------------
    if p["new_products"]:
        state["open_flags"]["new_products_no_location"] += len(p["new_products"])
    n = state["open_flags"]["new_products_no_location"]
    if n:
        days_to_count = (6 - day.weekday()) % 7 or 7
        urgent = days_to_count <= 2
        flags.append(flag("high" if urgent else "high", "NP",
                          "%d products are still purchased with no inventory location." % n,
                          "They land in cost and are never counted back out, so the "
                          "purchase reads as consumed. Next end of week count is in %d "
                          "days.%s" % (days_to_count,
                                       " Assign the locations before then."
                                       if urgent else "")))
